#1. In the table actor, what last names are not repeated? For example if you would sort the data in the table 
...
# output list.
select first_name, count(last_name) from actor group by first_name having count(last_name) = 1;
